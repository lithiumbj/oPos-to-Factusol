...
while ($fila = $resultado->fetch_assoc()) {
    ...
    echo 'Client - '.$fila['SEARCHKEY'].'<br>' ;ob_flush();
}
//Write clients
$objWriter = PHPExcel_IOFactory::createWriter($excel2, 'Excel5');
$objWriter->save('./output/CLI.xls');
/*
...
function CreateCliente($cod, $comptaCode, $nif, $name, $fiscalName, $address, $poblacion, $cp, $provincia, $country) {
    ...
    $excel2->getActiveSheet()->setCellValue("C" . $cliCounter, $nif);
    $excel2->getActiveSheet()->setCellValue("D" . $cliCounter, $name);
    $excel2->getActiveSheet()->setCellValue("E" . $cliCounter, $name);
    $excel2->getActiveSheet()->setCellValue("F" . $cliCounter, $address);
    $excel2->getActiveSheet()->setCellValue("G" . $cliCounter, $poblacion);
    $excel2->getActiveSheet()->setCellValue("H" . $cliCounter, intval($cp));
    $excel2->getActiveSheet()->setCellValue("I" . $cliCounter, $provincia);
    $excel2->getActiveSheet()->setCellValue("J" . $cliCounter, $country);
    $cliCounter++;
}
...
function createTicketsForUser() {
    ...
    while ($fila = $resultado->fetch_assoc()) {
        ...
        $excelTickets->getActiveSheet()->setCellValue("D" . $ticketCounter, date('d/m/Y', strtotime($fila['DATENEW'])));
        $excelTickets->getActiveSheet()->setCellValue("E" . $ticketCounter, 2);
        $excelTickets->getActiveSheet()->setCellValue("I" . $ticketCounter, $fila['SEARCHKEY']);
        $excelTickets->getActiveSheet()->setCellValue("I" . $ticketCounter, $fila['CUSTNAME']);
        $excelTickets->getActiveSheet()->setCellValue("S" . $ticketCounter, $fila['SUBTOTAL']);
        $ticketCounter++;
        ...
        while ($fila2 = $resultado2->fetch_assoc()) {
            ...
            $excelLines->getActiveSheet()->setCellValue("L" . $linesCounter, 3);
